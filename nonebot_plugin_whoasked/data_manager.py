...
class MessageRecorder:
    ...
    def _load_messages(self) -> dict[str, list[dict[str, Any]]]:
        ...
        try:
            ...
            with self.message_file.open("r", encoding="utf-8") as f:
                data = json.load(f)
                logger.info(
                    f"成功加载历史消息记录，共 {sum(len(v) for v in data.values())} 条消息"
                )
                return data
        except Exception as e:
    ...
    def _record_for_users(
        self, at_list: list[str], reply_user_id: str, message_info: dict[str, Any]
    ):
        """将消息记录给相关用户"""
        # 记录给被@的用户
    ...
    async def get_at_messages(self, user_id: str) -> list[dict[str, Any]]:
        ...
        # 返回按时间排序的最新消息
        return sorted(self.messages[user_id], key=lambda x: x["time"], reverse=True)[
            :max_messages
        ]
